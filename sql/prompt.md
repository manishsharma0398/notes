...
| 07 | `07-transactions-concurrency` | retrofitted **✓** |
...
**Next: `08-null-semantics-three-valued-logic`, retrofit.**

**Ch7 left defects behind, and the chapter contradicts itself.** `notes.md` §2 and `interview.md`
Q2 state the Postgres phantom behaviour correctly; `README.md` §4.C does not. `README.md` §4.A's
dirty-read example cannot happen in Postgres at all. Worst, §4.D justifies `SERIALIZABLE` with a
write-write conflict that **`REPEATABLE READ` already catches** — the case that actually needs it,
write skew, is nowhere in the chapter. All measured; see `HISTORY.md` 2026-09-11. Untouched per the
add-do-not-rewrite rule.
...
the retrofit of 08 and 12–17 is still outstanding.
...
Work in **chapter order** for what remains: 08 → 11 → 12 → … → 18.
(03, 04, 06, 07, 09 and 10 are done.)
