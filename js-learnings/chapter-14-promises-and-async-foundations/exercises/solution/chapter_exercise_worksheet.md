# Chapter 14 Worksheet — Promises as a Language Feature
...
mechanism: The executor runs immediately and synchronously, so A2 prints during new Promise(...). .then's handler is deferred to the microtask queue even though p is already fulfilled — run-to-completion means the current synchronous block finishes and the stack empties before any microtask runs, so A4 beats A3.

which line is scheduled rather than executed:
the callback function in .then
```
...
mechanism: one-way, permanent, subsequent calls are silent no-ops

what would happen if the executor threw AFTER resolve("a"): Still B fulfilled: a. The state is already locked, so the throw can't move it and the error is discarded entirely — not reported anywhere. Unlike the no-op reject, though, the throw does abort the rest of the executor. A throw before settling would reject normally.
```
...
what value does each handler RECEIVE, and why: each hander recieved 1. the base promise is then chained to multiple handlers with each recieving 1 as v. on C chain, Promise is resolved with 2 as .then also returns a promise and finally it is console logged.
```
...
Promise.resolve("H-value")
  .finally(() => "H-ignored")
  .then((v) => console.log("H:", v));
Promise.resolve("x")
  .finally(() => {
    throw new Error("H-thrown");
  })
  .catch((e) => console.log("H2:", e.message));
```
...
what exactly makes something a thenable: then method inside a promise object.
...
in js, await pauses the execution of lines below it , the lines below it in a block will now all be deferred until all the sync ops are executed, as the microstasks will then be able to push the handlers into call stack.

does `await 42` cost anything: it defers the execution of its below line until call stack is empty.
```
...
**C and L are the pair.** One is about _where the handler is attached_, the other about _when
the function was called_.
...
(async () => {
  ...
  async function* three() {
    for (let i = 1; i <= 3; i++) yield await delay(100, i);
  }
  const out = [];
...
(async () => {
  async function* counted() {
    ...
    while (true) {
      ...
      yield await delay(5, i);
    }
  }
  const out = [];
  for await (const v of counted()) {
    out.push(v);
...
(async () => {
  const items = [
    delay(30, "ok"),
    Promise.reject(new Error("item 2")),
    delay(30, "third"),
  ];
  try {
