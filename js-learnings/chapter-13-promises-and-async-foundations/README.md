# Chapter 13 — Asynchronous Foundations: Promises as a Language Feature
...
Almost every promise misconception dissolves once that lands:
...
This is the fact that separates people who use promises from people who understand them.
...
This is why an ORM row or a config object with a `then` field breaks in an `async` function in
a way that looks like magic. It is also the **interop** feature that let jQuery deferreds, `Q`
...
costs — are Chapter 14.** This chapter only claims: *never synchronous*.
...
**Empty-input edge cases** (interview bait, and the reason a "wait for zero jobs" path hangs):
...
**It isn't one, and this is the interview-grade detail.** While the loop is awaiting item 1,
...
**1. The floating promise.** `doWork();` with no `await` and no `.catch` — an async function
that fails after the request has already returned 200. Lint rules exist for this
(`no-floating-promises`) precisely because it is invisible in review.

**2. `array.forEach(async x => …)`.** `forEach` ignores return values, so nothing is awaited,
the loop "finishes" instantly, and errors become unhandled rejections. Use `for...of` with
`await`, or `Promise.all(array.map(async …))`.

**3. `await` in a loop over 500 items.** Correct, and 500× slower than it needs to be.

**4. Errors rejected as strings**, so the stack is gone by the time anyone reads the log.

**5. The accidental thenable** — an object with a `then` field, silently unwrapped by `await`.

**6. `try { f(); } catch` around an async call without `await`** — catches nothing.

**7. Mixing callbacks and promises in one function** so that some paths resolve twice. The
second resolve is a no-op, so it fails silently and *looks* like the wrong branch ran.
...
- **Chapter 14** answers every ordering question this chapter deliberately refused: microtasks
