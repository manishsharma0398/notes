...
3. **"More workers = better performance"**: False. Too many workers cause context switching overhead. Use `os.availableParallelism()` workers (one per CPU core). Note: this is preferred over `os.cpus().length` as it respects CPU affinity.
...
4. **Limit worker count**: Use `os.availableParallelism()` workers (preferred over `os.cpus().length`; one per available CPU)
...
10. **Use SharedArrayBuffer carefully**: Only for typed arrays; use `Atomics` for synchronization to avoid race conditions
...
```javascript
// Get CPU count
const numCPUs = os.availableParallelism(); // preferred over os.cpus().length
...
// Fork cluster worker
cluster.fork(); // Use cluster.isPrimary (cluster.isMaster is deprecated since v16)
