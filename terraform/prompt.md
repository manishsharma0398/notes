...
- `examples/` — runnable HCL with real `plan`/`apply` output pasted.
...
- `exercises/cumulative_exercise.md` — 1–3 hours, integrating everything so far. Prefer something
  that **doubles as a whiteboard question** at this level: a module you write, refactor and re-plan; a state-surgery exercise; a drift reproduction. Phased, with success
...
**Applies from the next chapter onward.** Chapters 1–20 were written under the older contract
